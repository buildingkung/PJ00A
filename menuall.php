...
$sql1 = "SELECT * FROM `food` where Id_food = $ran_1";
$result_1 = $conn->query($sql1);
$row_1 = mysqli_fetch_array($result_1);

$sql2 = "SELECT * FROM `food` where Id_food = $ran_2";
$result_2 = $conn->query($sql2);
$row_2 = mysqli_fetch_array($result_2);

$sql3 = "SELECT * FROM `food` where Id_food = $ran_3";
$result_3 = $conn->query($sql3);
$row_3 = mysqli_fetch_array($result_3);

$sql4 = "SELECT * FROM `food` where Id_food = $ran_4";
$result_4 = $conn->query($sql4);
$row_4 = mysqli_fetch_array($result_4);

$sql5 = "SELECT * FROM `food` where Id_food = $ran_5";
...
if (isset($_POST['bt11'])) {
    ...
    $sql = "SELECT * FROM `food` where type = 1";
    $result = $conn->query($sql);
} else if (isset($_POST['bt2'])) {
    $sql = "SELECT * FROM `food` where type = 2";
    $result = $conn->query($sql);
} else if (isset($_POST['bt3'])) {
    $sql = "SELECT * FROM `food` where type = 3";
    $result = $conn->query($sql);
} else if (isset($_POST['bt4'])) {
    $sql = "SELECT * FROM `food` where type = 4";
    $result = $conn->query($sql);
} else if (isset($_POST['bt5'])) {
    $sql = "SELECT * FROM `food` where type = 5";
    $result = $conn->query($sql);
} else if (isset($_POST['bt6'])) {
    $sql = "SELECT * FROM `food` where type = 6";
    $result = $conn->query($sql);
} else if (isset($_POST['bt7'])) {
    $sql = "SELECT * FROM `food` where type = 7";
    $result = $conn->query($sql);
} else if (isset($_POST['bt8'])) {
    $sql = "SELECT * FROM `food` where type = 8";
    $result = $conn->query($sql);
} else if (isset($_POST['bt9'])) {
    $sql = "SELECT * FROM `food` where type = 9";
    $result = $conn->query($sql);
} else if (isset($_POST['bt10'])) {
    $sql = "SELECT * FROM `food` where type = 10";
    $result = $conn->query($sql);
} else {
    $sql = "SELECT * FROM `food`";
    $result = $conn->query($sql);
...
<body class="animsition">
                            ...
                            <ul class="main_menu">
                                ...
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-content">
                                        ...
                                        <form method="post">
                                            <button name="bt1" style="color:rgb(255, 255, 255);font-size:17px;">ของทานเล่น</button>
                                            <button name="bt2" style="color:rgb(255, 255, 255);font-size:17px;">ของต้ม</button>
                                            <button name="bt3" style="color:rgb(255, 255, 255);font-size:17px;">ของผัด</button>
                                            <button name="bt4" style="color:rgb(255, 255, 255);font-size:17px;">ยำ</button>
                                            <button name="bt5" style="color:rgb(255, 255, 255);font-size:17px;">น้ำพริก</button>
                                            <button name="bt6" style="color:rgb(255, 255, 255);font-size:17px;">ไข่เจียว</button>
                                            <button name="bt7" style="color:rgb(255, 255, 255);font-size:17px;">ปลา</button>
                                            <button name="bt8" style="color:rgb(255, 255, 255);font-size:17px;">ของเส้น</button>
                                            <button name="bt9" style="color:rgb(255, 255, 255);font-size:17px;">ข้าว</button>
                                            <button name="bt10" style="color:rgb(255, 255, 255);font-size:17px;">ของหวาน</button>
            ...
            <li class="t-center m-b-13">
                <div class="dropdown1">
                    ...
                    <div class="dropdown1-content">
                        ...
                        <form method="post">
                            <button name="bt1" style="font-size:17px;">ของทานเล่น</button> </a>
                            <button name="bt2" style="font-size:17px;">ของต้ม</button>
                            <button name="bt3" style="font-size:17px;">ของผัด</button>
                            <button name="bt4" style="font-size:17px;">ยำ</button>
                            <button name="bt5" style="font-size:17px;">น้ำพริก</button>
                            <button name="bt6" style="font-size:17px;">ไข่เจียว</button>
                            <button name="bt7" style="font-size:17px;">ปลา</button>
                            <button name="bt8" style="font-size:17px;">ของเส้น</button>
                            <button name="bt9" style="font-size:17px;">ข้าว</button>
                            <button name="bt10" style="font-size:17px;">ของหวาน</button>
    ...
    <!-- Title Page -->
    <section>
        <img src="Foodlis/bg1.png" style="width:100%;height:50%;">
    </section>
            ...
            <div class="container">
                ...
                <div class="row">
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo "<img src='Foodlis/" . $row['imgname'] . "' alt='IMG-INTRO' id='" . $row['name'] . ":" . $row[4] . ":" . $row[0] . "'  onclick='a(this.id)'>";
                        echo "</div>";
                        $ddd = $row['name'] . ":" . $row[4];
                        if( $row[8] == 1){
                            echo "<button class='button4'>sold out <i class='fa fa-ban'></i></button>";
                        }
                        else
                        {
                         echo "<button id='" . $row['name'] . ":" . $row[4] . ":" . $row[0] . "' class='button' onclick='a(this.id)'>ใส่ตระกร้า <i class='fa fa-shopping-cart'></i></button>";
                        }
                        
                        echo "<div class='wrap-text-blo1 p-t-35'>";
                        
                        echo "<h4 class='txt5 color0-hov trans-0-4 m-b-13'>" . $row['name'] . " / " . $row[2] . "</h4>";
                        echo "<p class='m-b-20'>" . $row[4] . " Baht</p>";
            ...
            function a(id) {
                ...
                for (var count_split = string_split.length - 1; count_split > 0; count_split--) {
                    ...
                    if (string_split[count_split] != "") {
                        ...
                        cell2.innerHTML = "<center><input type='number' class='counter' min='0' id='myNumber" + string_split_1[2] + "' value='1' style='width:50px;'></center>";
